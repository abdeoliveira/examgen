...
\fancyfoot[RO,RE]{1/2022.12.27-23.09.04}
...
1. A particle of mass 1.2 kg is subject to an external force of 16.9 N. Calculate the acceleration in m/s$^2$ in a one-dimensional movement.\\
(a)14.1~
(b)4.3\\
(c)6.6~
(d)10.3\\
...
2. Consider the rectangle triangle of the figure below and knowing $\theta=28^{\circ}$, determine $\phi$ \textbf{in rad}.\\
...
(c) If $\vec{A}$ and $\vec{B}$ are vectors, then $\vec{A}\times\vec{B}$ is a vector perpendicular to both $\vec{A}$ and $\vec{B}$.\\
(d) Vectors can not be multiplied by scalars.\\
